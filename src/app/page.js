  ...
  const handleConvert = async () => {
    ...
    try {
      const res = await fetch('/api/generate-apk', {
        ...
        body: JSON.stringify({ url: url.trim(), appName: appName.trim() }),
      })
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || 'APK generation failed')
      }
    ...
    <div className="min-h-screen">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
            <div className="flex-1 w-full max-w-xl">
              <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-3">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary-100 text-primary-700 text-xs font-semibold uppercase tracking-wider">
                  <MdAutoAwesome className="text-sm" /> Free · Instant · No signup
                </span>
              </motion.div>
                  ...
                  {!done ? (
                    <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                      ...
                      <motion.button
                        ...
                        disabled={loading}
                        className="w-full py-3.5 rounded-xl bg-gradient-to-r from-primary-600 to-primary-500 text-white font-semibold flex items-center justify-center gap-2 shadow-lg shadow-primary-500/30 hover:shadow-primary-500/50 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                      >
                        {loading ? (
                          <>
      ...
      <div className="relative z-10 container mx-auto px-4 py-16 max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {STEPS.map((step, i) => (
            <StepCard key={step.title} number={i + 1} {...step} isLast={i === STEPS.length - 1} />
          ))}
        </div>
      </div>
