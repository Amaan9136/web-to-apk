...
export default function NgrokModal({ isOpen, onClose, ngrokData, apkName }) {
  ...
  const copyUrl = () => {
    navigator.clipboard.writeText(ngrokData?.downloadUrl || '')
    setCopied(true)
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-primary-500 to-violet-500 flex items-center justify-center">
                  <MdQrCode2 className="text-white text-lg" />
                </div>
                <div>
                  <h3 className="font-bold text-slate-800 text-sm">Scan to Download</h3>
                  <p className="text-slate-400 text-xs">Temporary ngrok link</p>
                </div>
              </div>
              <button onClick={onClose} className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors">
                <MdClose className="text-slate-600" />
              </button>
            </div>
            ...
            <div className="flex justify-center mb-4">
              {ngrokData?.qr ? (
                <div className="qr-container shadow-lg">
                  <Image src={ngrokData.qr} alt="QR Code" width={180} height={180} />
                </div>
              ) : (
            ...
            <div className="text-center mb-4">
              <div className="flex items-center gap-1 justify-center mb-1">
                <MdPhoneAndroid className="text-primary-500 text-sm" />
                <span className="text-slate-700 text-sm font-semibold">{apkName}.apk</span>
              </div>
            ...
            <button
              onClick={copyUrl}
              className="w-full py-2.5 rounded-xl bg-gradient-to-r from-primary-600 to-primary-500 text-white font-semibold flex items-center justify-center gap-2 text-sm shadow-md hover:shadow-lg transition-all"
            >
              {copied ? <><MdCheckCircle className="text-lg" /> Copied!</> : <><MdContentCopy className="text-lg" /> Copy Download Link</>}
            </button>

            <p className="text-center text-xs text-slate-400 mt-3">⚠️ Temporary — expires when server restarts</p>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
